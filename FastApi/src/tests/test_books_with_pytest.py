...
async def test_create_book(db_session, async_client):
    seller_user = seller.Seller(first_name="Mister", last_name="Smith", email="mtsshad@yandex.ru", password="123456")
    db_session.add(seller_user)
    await db_session.flush()
    data = {"title": "Wrong Code", "author": "Robert Martin", "pages": 104, "year": 2007, "seller_id" : seller_user.id}
...
async def test_get_books(db_session, async_client):
    ...
    book = books.Book(author="Pushkin", title="Eugeny Onegin", year=2001, count_pages=104, seller_id=seller_user.id)
    book_2 = books.Book(author="Lermontov", title="Mziri", year=1997, count_pages=104, seller_id=seller_user.id)
    ...
    assert response.json() == {
        "books": [
            {"title": "Eugeny Onegin", "author": "Pushkin", "year": 2001, "id": book.id, "count_pages": 104, "seller_id" : seller_user.id},
            {"title": "Mziri", "author": "Lermontov", "year": 1997, "id": book_2.id, "count_pages": 104, "seller_id" : seller_user.id},
        ]
    }
...
async def test_update_book(db_session, async_client):
    ...
    response = await async_client.put(
        ...
        json={"title": "Mziri", "author": "Lermontov", "count_pages": 100, "year": 2007, "id": book.id, "seller_id" : seller_user.id},
    )
    ...
    assert res.seller_id == seller_user.id
